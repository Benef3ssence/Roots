<?php
  ...
  $app->get('/my-district/?', function () use ($app) {    
        $controller = new UF\MyDistrictController($app);
             
               return      $controller->view();
        });

  $app->get('/budget/?', function () use ($app) {    
        $controller = new UF\BudController($app);
  ...
  $app->get('/legislation/?', function () use ($app) {    
        $controller = new UF\ColegController($app);
             
               return      $controller->viewl();
        });


  $app->get('/oversight/?', function () use ($app) {    
        $controller = new UF\OsiController($app);
             
               return      $controller->viewo();
        });
  $app->get('/delegation/?', function () use ($app) {    
        $controller = new UF\DelegateController($app);
             
               return      $controller->view();
        });
$app->get('/Participatory/?', function () use ($app) {    
       $controller = new UF\PBideaController($app);
              
   
            // Do what we need to do to render the page containing the form in the GET route
        });

 $app->get('/Participatory/idea/?', function () use ($app) {    
      // Access-controlled page
         
            $controller = new UF\PBideaController($app);
          return $controller->pageParticipatory();
        });
        
     $app->post('/Participatory/idea/?', function () use ($app) {    

                $controller = new UF\PBideaController($app);
          return $controller->PBidea();
        });
